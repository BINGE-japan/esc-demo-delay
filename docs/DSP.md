...
縦=タイプの択一パターンを、再生中の拍に当たるステップで適用。**内部解像度は16分**（`bars×16`・最大64 スロット）だが **UI は8分カラム表示**（1クリック=2スロット=8分セル・最小16分・横連結=長さ）。**ループ長＝`Bars(288)`(1/2/4 小節)** で可変＝パターンでループ＝**再現性**。拍ロックは曲タイムライン（VST）/再生開始基準（Web）。**セル値 raw= ベース型(下位3bit) | Dive(bit3=8)**。ベース: **0=Dry / 1=Glitch / 2=Freeze / 3=Reverse / 4=Mute / 5=Repeat**（排他）。**Dive** はベース出力に**重ねがけ**できるモディファイア（Mute 不可）。隣接ブロックは raw 全体で判定（Dive 違いは別ブロック）。
