...
**ランダム配置**: DSP のランダムモードは撤去（2026-06-22）。**UI(StepGrid)の Random/▶ ボタン**がシード(前進)から、**◀** が前のシード（直前のランダムに戻る）でセルを `steps[]` に書く＝**実セルなので拍ロック・再現性・編集はそのまま**。`#seed` 表示・Clear で全消去。worklet はグリッドを素直に再生するだけ。**VST はキー入力が runtime に取られ DOM 入力欄に届かない（ネイティブ層の制約）ためシードはボタン操作のみ**。**重み付き生成**: ベースはラン単位（**Mute は必ず単発16分＝両隣を空ける**＋やや高頻度 / 他は1-3セル）、**Dive は8分(2セル)が最頻＋残りランダム長(3-6)**の連続ランで重ね、**空セルに単発16分 Glitch を追加(隣接Glitchは不可)**（密度/重みは StepGrid の定数で耳調整）。
...
Freeze:  ブロック頭で直近 FREEZE_LOOP_MS(=126ms) を掴み、**オーバーラップ crossfade ループ**で持続音化。
         2読み位置を L/2 ずらし Hann 窓（w1+w2=1）で混ぜる＝継ぎ目クリックもチャンクのアタック起伏も平坦化
         （短いほどループ周期が可聴ピッチ域＝持続トーン寄り）。→ **原音(dry)追従ゲイン**（出力を inEnv×Level
         に正規化＝入力でバラつかず一定。Level=0.55・上限 +15dB クランプ）→ **ピンクノイズ**(エア・Noise=0.28・
         Kellet economy で -3dB/oct) を加算 → **後段 EQ**（RBJ biquad ×3: HP370 → ピーク1.2k(+14dB Q2.2)
         → ハイシェルフ3k(-7.2dB Q1)）。耳で確定し定数化（2026-06-22）。凍結ごとに EQ・ピンク状態をリセット
