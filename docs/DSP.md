...
**ランダム配置**: DSP のランダムモードは撤去（2026-06-22）。**UI(StepGrid)の Random ボタン**がシード(表示・押すたびに前進)からセルを `steps[]` に書く＝**実セルなので拍ロック・再現性・編集はそのまま**。Clear ボタンで全消去。worklet はグリッドを素直に再生するだけ。**重み付き生成**: ベースはラン単位（**Mute は単発16分寄り**＋やや高頻度 / 他は1-3セル）、**Dive は2個以上(8分+)の連続ラン**で重ねる（密度/重みは StepGrid の定数で耳調整）。
