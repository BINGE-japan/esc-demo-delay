...
| [src/components/StepGrid.vue](../src/components/StepGrid.vue)                   | Glitch シーケンサ UI（小節数タブ＋bars×16 カラム・1セル16分・ドラッグ伸縮・Dive重ね/Mute最下段） | 中       |
...
- 既存: synth `0..5` / saturator `100..102`。本プラグインは **200番台**:
  ...
  - `grid`（自動スライダ外・StepGrid 描画）: Bars=288（ループ長 1/2/4 小節）/ Step 1–64 = 223–286（raw 0..13＝ベース(0Dry/1Glitch/2Freeze/3Reverse/4Mute/5Repeat)|Dive(8)。16分カラム表示）
