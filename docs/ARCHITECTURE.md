...
- **`grid` フラグ**: `useParam` ハンドルは作る（StepGrid が読み書き＋VST 自動化）が**自動スライダ UI には出さない**。`hidden`（ハンドル自体作らない）との中間。Glitch の step0..15 が該当＝`StepGrid.vue` が描画。
...
- **glitchPhase も transport 駆動**: hidden param。App が再生中 rAF で `positionSamples`(VST)/`ctx.currentTime`(Web) から小節内位相(0..1)を算出→`applyParam('glitchPhase', …)`。worklet はこれにサンプル精度で同期し Glitch ステップを拍ロック（大ドリフトのみスナップ）。停止中はホールド。
...
- 既存: synth `0..5` / saturator `100..102`。本プラグインは **200番台**:
  ...
  - ステップ（`grid`・enum 0..5）: Step 1–16 = 223–238（Glitch シーケンサ。0Dry(空)/1Glitch/2Freeze/3Reverse/4Mute/5Repeat。useParam あり・自動スライダなし）
