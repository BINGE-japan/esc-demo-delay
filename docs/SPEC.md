...
| 212     | Spectral Fill | 0 / 1        | OFF  | トグル     | —    | グリッチ   | Random gate 無音に**スペクトル反転音**を差し込む                       |
| 217     | Glitch On     | 0 / 1        | ON   | トグル     | —    | グリッチ   | グリッチセクションの ON/OFF                                            |
| 223–238 | Step 1–16     | 0 〜 4       | 0    | 連続(enum) | —    | グリッチ   | 各16分ステップの type: 0Dry/1Repeat/2Freeze/3Reverse/4Random（`grid`） |
...
- **Glitch（ステップシーケンサ）**: **横=16分ステップ(1小節)/縦=タイプ**のマス目。各ステップ択一: **Dry / Repeat(ラチェット) / Freeze / Reverse / Random**。BPM同期・**拍ロック**（VST=曲の小節頭=ステップ1、Web=再生開始基準）でパターンがループ＝**再現性**。Random は既存シード付き stutter/gate＝"ランダム質感だが決定論"。`Glitch(204)`=全体 wet（空グリッド=全Dryで透過）、`Spectral Fill(212)`=Random gate に **(-1)ⁿ スペクトル反転音**。完全ランダムトグルは後日（[DSP.md](./DSP.md) §3 Glitch）。
