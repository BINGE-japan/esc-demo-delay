...
- **Freeze（Ice Reverb）**: 凍結域のグラニュラー雲を励起源に **FDN 拡散残響**（入力 allpass×2 → 4ライン Hadamard FDN・damping）を足し、**Guitar Rig Iceverb 風の“コー”**（拡散した氷/金属的な持続残響）。**iceberg HP は 4-pole(24dB/oct・310Hz 固定)** に強化＝低域をしっかり削る。FDN 係数は**耳で確定して定数化**（Decay 0.12 / Diffuse 0.70 / Size 0.31 / Tone 0.73 / Mix 0.14。旧デバッグ param 291-295 は撤去）（[DSP.md](./DSP.md) §3 Freeze）。
